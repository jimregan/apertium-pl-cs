...
\label{fig:pardefs}\\
\small{Each \textbf{e}ntry contains a \textbf{p}air, where the \textbf{l}eft contains
the suffix to be analysed/generated, and the \textbf{r}ight contains the
suffix of the lemma, along with the set of \textbf{s}ymbols to be attached.}}
\end{figure*}
\begin{figure*}
\begin{small}
\begin{verbatim}
...
None of the Apertium ``incubator'' materials were at a particularly advanced stage of development; in addition, both 
...
Adjectives usuall preceed the noun in both Czech and Polish, but in certain fixed 
multiwords in Polish, the adjective follows the noun (usually, the adjectives in
these multiwords describe an inherent property). A separate project to add
...
modules, to allow backoff to the previous longest match when an exception
to the rule currently being processed has been detected, but the change is
still being tested\footnote{The version being tested is available from Apertium's SVN repository:
